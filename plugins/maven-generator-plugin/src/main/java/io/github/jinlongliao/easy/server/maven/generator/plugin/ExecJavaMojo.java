package io.github.jinlongliao.easy.server.maven.generator.plugin;
...
import io.github.jinlongliao.easy.server.common.generator.plugin.exec.CodeGenerate;
 ...
 * @since 1.0
 */
@Mojo(name = "java", threadSafe = true, requiresDependencyResolution = ResolutionScope.COMPILE_PLUS_RUNTIME)
public class ExecJavaMojo
     ...
     * @since 1.0
     */
    @Parameter(required = true, property = "exec.mainClass", defaultValue = "io.github.jinlongliao.common.generator.plugin.exec.CodeGenerate")
    ...
    public void execute()
        ...
        Thread bootstrapThread = new Thread(threadGroup, () -> {
            if (mainClass.equals("io.github.jinlongliao.common.generator.plugin.exec.CodeGenerate")) {
                CodeGenerate.run(arguments, classLoader);
            } else {
                ...
                try {
                    ...
                    mainHandle.invoke(Arrays.stream(arguments).toArray());
                    ...
                    Thread.currentThread().getThreadGroup().uncaughtException(Thread.currentThread(),
                        ...
                        InvocationTargetException e) { // use the cause if available to improve the plugin execution output
        ...
        for (Collection<Thread> threads = getActiveThreads(threadGroup); !threads.isEmpty(); threads =
            ...
            for (Thread thread : threads) {
                ...
                if (stopUnresponsiveDaemonThreads) {
                    getLog().warn("thread " + thread + " will be Thread.interrupt()'ed");
                    thread.interrupt();
                } else {
