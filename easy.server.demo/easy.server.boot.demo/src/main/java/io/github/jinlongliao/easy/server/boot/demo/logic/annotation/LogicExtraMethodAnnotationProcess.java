...
public class LogicExtraMethodAnnotationProcess implements ExtraMethodAnnotationProcess {
    @Override
    public ExtraMethodDesc extraProcessMethod(Map<String, MethodInfo> data, Method method) {
        ...
        List<ExtraMethodDesc.MethodDesc> desc = Arrays.stream(logic.value())
                .map(node -> new ExtraMethodDesc.MethodDesc(node.getLogicId(), node.getDesc()))
